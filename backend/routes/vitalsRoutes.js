...
module.exports = function (broadcast) {
  ...
  router.post('/', async (req, res) => {
    try {
      const { deviceId, heartRate, spo2, userId } = req.body;
      if (!deviceId || heartRate == null || spo2 == null) {
        return res.status(400).json({ error: 'deviceId, heartRate, and spo2 are required' });
      }

      const reading = await VitalsReading.create({ deviceId, heartRate, spo2, userId });

      // Push to any connected dashboards in real time
      broadcast({ type: 'vitals', data: reading });
  ...
  router.get('/latest', async (req, res) => {
    ...
    const reading = await VitalsReading.findOne({ deviceId }).sort({ timestamp: -1 });
    res.json(reading || {});
  });
